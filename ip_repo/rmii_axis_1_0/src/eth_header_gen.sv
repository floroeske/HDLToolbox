...
// Revision 0.02 - Added dynamic IPv4 header checksum calculation
// Additional Comments:
//
//////////////////////////////////////////////////////////////////////////////////
...
// Calculate IPv4 header checksum dynamically
logic [15:0] calculated_checksum;
...
function automatic [15:0] calc_ipv4_checksum(
    ...
    // Add all 16-bit words of the IPv4 header (excluding checksum field)
    // The checksum field itself is treated as 0x0000 during calculation
    sum = 0;
    
    // Word 0: Version/IHL + DSCP/ECN
    sum = sum + {VERSION_IHL, DCSP_ECN};
    ...
    sum = sum + FLAGS_FRAGMENT_OFFSET;
    
    // Word 4: TTL + Protocol
    sum = sum + {TIME_TO_LIVE, PROTOCOL};
    
    // Word 5: Header Checksum = 0x0000 (skipped)
    
    // Word 6-7: Source IP Address
    sum = sum + FPGA_IP[31:16];
    sum = sum + FPGA_IP[15:0];
    
    // Word 8-9: Destination IP Address
    sum = sum + HOST_IP[31:16];
    sum = sum + HOST_IP[15:0];
    
    // Add carry bits (fold 32-bit sum to 16-bit)
    // Maximum of 2 iterations needed for 10 words
...
assign calculated_checksum = calc_ipv4_checksum(IPV4_LENGTH);
...
assign header.ipv4.header_checksum = {<<8{calculated_checksum}};
